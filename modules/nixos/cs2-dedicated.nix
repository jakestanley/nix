...
let
  ...
  gsltTokenFileValue = if cfg.gsltTokenFile == null then "" else cfg.gsltTokenFile;
  requiredConditionPaths =
    lib.optional (cfg.environmentFile != null) cfg.environmentFile
    ++ lib.optional (cfg.requireRconPassword && cfg.rconPasswordFile != null) cfg.rconPasswordFile
    ++ lib.optional (cfg.requireGsltToken && cfg.gsltTokenFile != null) cfg.gsltTokenFile;
  ...
  launchScript = pkgs.writeShellScript "cs2-dedicated-launch" ''
    ...
    gslt_token_file=${lib.escapeShellArg gsltTokenFileValue}
    if [[ -z "$gslt_token" && -n "$gslt_token_file" && -r "$gslt_token_file" ]]; then
      gslt_token="$(<"$gslt_token_file")"
    fi

    gslt_args=()
    if [[ -n "$gslt_token" ]]; then
      gslt_args=(+sv_setsteamaccount "$gslt_token")
    elif [[ "${if cfg.requireGsltToken then "1" else "0"}" == "1" ]]; then
      echo "cs2-dedicated: no GSLT token provided; refusing to start (set GSLT_TOKEN or configure services.cs2Dedicated.gsltTokenFile)." >&2
      exit 1
    fi
...
in
{
  options.services.cs2Dedicated = {
    ...
    gsltTokenFile = lib.mkOption {
      type = lib.types.nullOr lib.types.str;
      default = null;
      example = "/run/secrets/cs2-gslt-token";
      description = ''
        Optional runtime path that stores the GSLT token as plain text.
        If `GSLT_TOKEN` is present in `environmentFile`, it takes precedence.
      '';
    };
    ...
    requireGsltToken = lib.mkOption {
      type = lib.types.bool;
      default = false;
      description = ''
        Require a GSLT token at startup. When enabled, service startup
        fails unless `GSLT_TOKEN` or `gsltTokenFile` is provided.
      '';
    };
    ...
    environmentFile = lib.mkOption {
      ...
      description = ''
        ...
        Supported keys: `RCON_PASSWORD`, `GSLT_TOKEN`.
      '';
    };
  ...
  config = lib.mkIf cfg.enable (lib.mkMerge [
    {
      assertions = [
        {
          assertion = (!cfg.requireRconPassword) || (cfg.rconPasswordFile != null) || (cfg.environmentFile != null);
          message = "services.cs2Dedicated.requireRconPassword is true but no rconPasswordFile or environmentFile is configured.";
        }
        {
          assertion = (!cfg.requireGsltToken) || (cfg.gsltTokenFile != null) || (cfg.environmentFile != null);
          message = "services.cs2Dedicated.requireGsltToken is true but no gsltTokenFile or environmentFile is configured.";
        }
      ];
    }
